...
class Service
{
    ...
    public function searchResponse($params, $model)
    {
        ...
        $flag_trash = $params['flag_trash'] ?? '';
        if ($flag_trash !== '') {
            $flag_trash = intval($flag_trash);
            if ($flag_trash === -1) $model = $model->onlyTrashed();
            if ($flag_trash === 1) $model = $model->withTrashed();
        }
    ...
    public function cleanTime($params, $columns = [])
    {
        foreach ($columns as $column) if (!empty($params[$column])) $params[$column] = unformat_time($params[$column]);
        return $params;
    }

    public function curl_request($url, $method, $fields = '', $header = [])
    {
        $curl = curl_init();
