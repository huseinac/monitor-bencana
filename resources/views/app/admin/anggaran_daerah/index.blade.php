...
@section('content')
    <div class="page-header">
        <div>
            ...
            <h1>Anggaran Daerah</h1>
            <p>Kelola data sektor terdampak bencana alam</p>
        </div>
        <button class="btn-primary-custom" onclick="info()" type="button">
            <i class="bi bi-plus-lg"></i> Tambah Anggaran Daerah
        </button>
    </div>

    <div class="panel">
        <div class="panel-header">
            <h5><i class="bi bi-table"></i> Data Anggaran Daerah</h5>
...
@push('scripts')
    <script>
        ...
        let selected_page = 1, _token = '{{ csrf_token() }}', base_url = '{{ route('anggaran_daerah.index') }}', params_url = '{{ $params ?? '' }}';
        ...
        let init_form = (id = '') => {
            ...
            $form_info.submit((e) => {
                ...
                $.ajax({
                    ...
                    contentType: false,
                    success: () => {
                        if (id === '') showToast('Anggaran Daerah berhasil ditambahkan', 'success');
                        else showToast('Anggaran Daerah berhasil diubah', 'success');
