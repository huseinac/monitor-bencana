...
@section('content')
    <div class="page-header">
        <div>
            ...
            <h1>User Program</h1>
            <p>Kelola akun user dan akses setiap user</p>
        </div>
    ...
    <div class="panel">
        <div class="panel-header">
            ...
            <form class="d-flex align-items-center gap-2" id="form_search">
                ...
                <x-select name="akses" class="form-select-sm" :options="array_combine($list_akses, $list_akses)" caption="Cari Akses" style="width:150px;font-size:.8rem;border-radius:8px;" onchange="search_data()" />
            </form>
        </div>
...
@push('scripts')
    <script>
        ...
        let selected_page = 1, _token = '{{ csrf_token() }}', base_url = '{{ route('user.index') }}', params_url = '{{ $params ?? '' }}';
        ...
        let init_form = (id = '') => {
            ...
            $form_info.submit((e) => {
                ...
                $.ajax({
                    ...
                    contentType: false,
                    success: () => {
                        if (id === '') showToast('User berhasil ditambahkan', 'success');
                        else showToast('User berhasil diubah', 'success');
                        init()
                    },
                }).fail((xhr) => {
