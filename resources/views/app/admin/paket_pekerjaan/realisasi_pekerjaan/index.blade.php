@extends('app.admin.paket_pekerjaan.detail')

@section('title')
    Realisasi Pekerjaan - {{ $paket_pekerjaan->nama }} -
@endsection
...
@section('sub_content')
    <div class="page-header">
        <div>
            ...
            <h1>Realisasi Pekerjaan</h1>
            <p>List item pekerjaan untuk paket pekerjaan</p>
        </div>
        <button class="btn-primary-custom" onclick="info()" type="button">
            <i class="bi bi-plus-lg"></i> Tambah Realisasi
        </button>
    </div>

    <div class="panel">
        <div class="panel-header">
            <h5><i class="bi bi-table"></i> Data Penyedia</h5>
...
@push('scripts')
    <script>
        ...
        let selected_page = 1, _token = '{{ csrf_token() }}', base_url = '{{ route('paket_pekerjaan.realisasi_pekerjaan.index', $paket_pekerjaan->id) }}', params_url = '{{ $params ?? '' }}';
        ...
        let init_form = (id = '') => {
            ...
            $form_info.submit((e) => {
                ...
                $.ajax({
                    ...
                    contentType: false,
                    success: () => {
                        if (id === '') showToast('Penyedia berhasil ditambahkan', 'success');
                        else showToast('Penyedia berhasil diubah', 'success');
                        init()
                    },
                }).fail((xhr) => {
