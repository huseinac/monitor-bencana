...
@section('content')
    <div class="page-header">
        <div>
            ...
            <h1>Perbaikan</h1>
            <p>Kelola kategori perbaikan perbaikan wilayah terdampak bencana</p>
        </div>
        <button class="btn-primary-custom" onclick="info()" type="button">
            <i class="bi bi-plus-lg"></i> Tambah Perbaikan
        </button>
    </div>

    <div class="panel">
        <div class="panel-header">
            <h5><i class="bi bi-table"></i> Data Perbaikan</h5>
...
@push('scripts')
    <script>
        ...
        let selected_page = 1, _token = '{{ csrf_token() }}', base_url = '{{ route('perbaikan.index') }}', params_url = '{{ $params ?? '' }}';
        ...
        let init_form = (id = '') => {
            ...
            $form_info.submit((e) => {
                ...
                $.ajax({
                    ...
                    contentType: false,
                    success: () => {
                        if (id === '') showToast('Perbaikan berhasil ditambahkan', 'success');
                        else showToast('Perbaikan berhasil diubah', 'success');
                        init()
                    },
                }).fail((xhr) => {
