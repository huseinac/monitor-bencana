...
@section('content')
    <div class="page-header">
        <div>
            ...
            <h1>Wilayah</h1>
            <p>Kelola data wilayah indonesia</p>
        </div>
    ...
    <div class="panel">
        <div class="panel-header">
            <h5><i class="bi bi-table"></i> Data Wilayah</h5>
            <form class="d-flex align-items-center gap-2" id="form_search">
                @csrf
                <div class="search-box">
                    <i class="bi bi-search"></i>
                    <x-input name="nama" class="form-control-sm" caption="Cari nama" onchange="search_data()" />
                </div>
                <div style="width: 150px;">
                    <x-input name="kode" prefix="search_" class="form-control-sm" caption="Cari Kode" onchange="search_data()" />
                </div>
            </form>
        </div>
...
@push('scripts')
    <script>
        ...
        let selected_page = 1, _token = '{{ csrf_token() }}', base_url = '{{ route('wilayah.index') }}', params_url = '{{ $params ?? '' }}';
        ...
        let init_form = (id = '') => {
            ...
            $form_info.submit((e) => {
                ...
                $.ajax({
                    ...
                    contentType: false,
                    success: () => {
                        if (id === '') showToast('Wilayah berhasil ditambahkan', 'success');
                        else showToast('Wilayah berhasil diubah', 'success');
                        init()
                    },
                }).fail((xhr) => {
