...
@section('content')
    <div class="page-header">
        <div>
            ...
            <h1>Masalah Kritis</h1>
            <p>Kelola data sektor terdampak bencana alam</p>
        </div>
        <button class="btn-primary-custom" onclick="info()" type="button">
            <i class="bi bi-plus-lg"></i> Tambah Masalah Kritis
        </button>
    </div>

    <div class="panel">
        <div class="panel-header">
            <h5><i class="bi bi-table"></i> Data Masalah Kritis</h5>
...
@push('scripts')
    <script>
        ...
        let selected_page = 1, _token = '{{ csrf_token() }}', base_url = '{{ route('masalah_kritis.index') }}', params_url = '{{ $params ?? '' }}';
        ...
        let init_form = (id = '') => {
            ...
            $form_info.submit((e) => {
                ...
                $.ajax({
                    ...
                    contentType: false,
                    success: () => {
                        if (id === '') showToast('Masalah Kritis berhasil ditambahkan', 'success');
                        else showToast('Masalah Kritis berhasil diubah', 'success');
