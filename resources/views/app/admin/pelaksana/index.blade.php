...
@section('content')
    <div class="page-header">
        <div>
            ...
            <h1>Pelaksana</h1>
            <p>Kelola data pelaksana dari Kementrian atau Perusahaan Milik Negara</p>
        </div>
        <button class="btn-primary-custom" onclick="info()" type="button">
            <i class="bi bi-plus-lg"></i> Tambah Pelaksana
        </button>
    </div>

    <div class="panel">
        <div class="panel-header">
            <h5><i class="bi bi-table"></i> Data Pelaksana</h5>
...
@push('scripts')
    <script>
        ...
        let selected_page = 1, _token = '{{ csrf_token() }}', base_url = '{{ route('pelaksana.index') }}', params_url = '{{ $params ?? '' }}';
        ...
        let init_form = (id = '') => {
            ...
            $form_info.submit((e) => {
                ...
                $.ajax({
                    ...
                    contentType: false,
                    success: () => {
                        if (id === '') showToast('Pelaksana berhasil ditambahkan', 'success');
                        else showToast('Pelaksana berhasil diubah', 'success');
                        init()
                    },
                }).fail((xhr) => {
